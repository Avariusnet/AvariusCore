...
void CouponHandler::insertNewCouponCodeinDB(std::string code, int itemid, int itemquantity, int used, int useablequantity)
{
...
std::string CouponHandler::createNewCouponCode()
{
	auto randchar = []() -> char
	{
		const char charset[] =
...
void CouponHandler::insertNewPlayerUsedCode(std::string charactername, int accountid, std::string couponcode)
{
	SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
PreparedQueryResult CouponHandler::getRequestedCodeData(std::string couponcode)
{
...
bool CouponHandler::hasPlayeralreadyUsedCode(std::string couponcode, int accountid)
{
...
bool CouponHandler::isItemCodeStillValid(std::string couponcode)
{
	PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_ITEMCODE);
	stmt->setString(0, couponcode);
	PreparedQueryResult result = CharacterDatabase.Query(stmt);
...
void CouponHandler::updateCouponCodeUsed(int used, std::string couponcode)
{
	SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
int CouponHandler::getFortuneItem()
{
	srand(time(NULL));
	int r = rand();
	
	if (r % 25 == 0) {
		int reward = getCouponRewardbyID(0);
		return reward;
	}

	if (r % 25 == 1) {
		int reward = getCouponRewardbyID(1);
		return reward;
	}

	if (r % 25 == 2) {
		int reward = getCouponRewardbyID(2);
		return reward;
	}

	if (r % 25 == 3) {
		int reward = getCouponRewardbyID(3);
		return reward;
	}

	if (r % 25 == 4) {
		int reward = getCouponRewardbyID(4);
		return reward;
	}

	if (r % 25 == 5) {
		int reward = getCouponRewardbyID(5);
		return reward;
	}

	if (r % 25 == 6) {
		int reward = getCouponRewardbyID(6);
		return reward;
	}

	if (r % 25 == 7) {
		int reward = getCouponRewardbyID(7);
		return reward;
	}

	if (r % 25 == 8) {
		int reward = getCouponRewardbyID(8);
		return reward;
	}

	if (r % 25 == 9) {
		int reward = getCouponRewardbyID(9);
		return reward;
	}

	if (r % 25 == 10) {
		int reward = getCouponRewardbyID(10);
		return reward;
	}

	if (r % 25 == 11) {
		int reward = getCouponRewardbyID(11);
		return reward;
	}

	if (r % 25 == 12) {
		int reward = getCouponRewardbyID(12);
		return reward;
	}

	if (r % 25 == 13) {
		int reward = getCouponRewardbyID(13);
		return reward;
	}

	if (r % 25 == 14) {
		int reward = getCouponRewardbyID(14);
		return reward;
	}

	if (r % 25 == 15) {
		int reward = getCouponRewardbyID(15);
		return reward;
	}

	if (r % 25 == 16) {
		int reward = getCouponRewardbyID(16);
		return reward;
	}

	if (r % 25 == 17) {
		int reward = getCouponRewardbyID(17);
		return reward;
	}

	if (r % 25 == 18) {
		int reward = getCouponRewardbyID(18);
		return reward;
	}

	if (r % 25 == 19) {
		int reward = getCouponRewardbyID(19);
		return reward;
	}

	if (r % 25 == 20) {
		int reward = getCouponRewardbyID(20);
		return reward;
	}

	if (r % 25 == 21) {
		int reward = getCouponRewardbyID(21);
		return reward;
	}

	if (r % 25 == 22) {
		int reward = getCouponRewardbyID(22);
		return reward;
	}

	if (r % 25 == 23) {
		int reward = getCouponRewardbyID(23);
		return reward;
	}

	if (r % 25 == 24) {
		int reward = getCouponRewardbyID(24);
		return reward;
	}
...
int CouponHandler::getCouponRewardbyID(int modulo)
{
...
void CouponHandler::playerRedeemCommand(Player * player, const char * args)
{
	...
	couponCodeStillValid = isItemCodeStillValid(couponCode);
	...
	bool codeUsed = hasPlayeralreadyUsedCode(couponCode, player->GetSession()->GetAccountId());
	if (player->GetSession()->GetSecurity() >= 2) {
...
void CouponHandler::couponGenerationperCommand(Player * player, const char * args)
{
...
void CouponHandler::playerCouponGerationForAFriend(Player * player, std::string logmessage)
{
	CustomCharacterSystem * CharacterSystem = 0;
...
void CouponHandler::playerCouponGenerationAndRedeeming(Player * player, std::string logmessage)
{
	CustomCharacterSystem * CharacterSystem = 0;
	CustomPlayerLog * PlayerLog = 0;
