...
CharacterSystemHandler* CharacterSystemHandler::instance()
{
...
PreparedQueryResult CharacterSystemHandler::getAccountbyID(int accountid)
{
...
void CharacterSystemHandler::insertNewFirstCharacterforPlayerCount(int guid, std::string charactername, int accountid, std::string accountname, int guildid, std::string ip)
{
...
PreparedQueryResult CharacterSystemHandler::getFirstCharacterPlayerLog(int accountid)
{
...
bool CharacterSystemHandler::hasPlayerAlreadyAFirstChar(int accountid,std::string action_done)
{
	PreparedStatement * stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_FIRST_CHAR_PLAYERLOG);
	stmt->setInt32(0, accountid);
...
bool CharacterSystemHandler::countIfPlayerHasLessTotalOf2FirstCharacters(int accountid)
{
...
int CharacterSystemHandler::getGuildCreateDate(int guildid)
{
...
//If Returnvalue = 0 no guild was found!
int CharacterSystemHandler::getGuildMemberCount(int guildid)
{
...
bool CharacterSystemHandler::hasPlayerAlreadyCharacters(int accountid)
{
...
void CharacterSystemHandler::executeFirstCharacter(Player * player, std::string playerlog)
{
...
std::string CharacterSystemHandler::getLastIPbyAccount(int accountid)
{
...
void CharacterSystemHandler::playerSetSingleFirstCharacter(Player * player)
{
...
void CharacterSystemHandler::playerSetGuildFirstCharacter(Player * player)
{
	bool hasPlayerAlreadyFirstChar = true;
...
void CharacterSystemHandler::playerGiveFirstCharacter(Player * player)
{
	bool hasPlayerAlreadyFirstChar = true;
...
void CharacterSystemHandler::deleteFirstCharacterPlayerLog(int accountid)
{
	SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
void CharacterSystemHandler::updateCharacterToZeroAccount(std::string newname,int guid)
{
	SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
std::string CharacterSystemHandler::generateNewCharacterName()
{
	auto randchar = []() -> char
	{
		const char charset[] =
...
void CharacterSystemHandler::insertNewPlayerPlayTimeReward(int playtime, std::string charactername, int guid)
{	
	SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
bool CharacterSystemHandler::checkIfPlayerGetPlayTimeReward(int playtime, int guid)
{
...
void CharacterSystemHandler::completeAddPlayTimeReward(int playtime, Player* player, int money, int rewarditem, std::string message)
{
	PreparedStatement *stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_PLAYTIME_REWARDS);
	stmt->setInt32(0, playtime);
	...
	if (!result) {
		...
		if (playtime == 1200) {
			...
			sPlayerLog->addCompletePlayerLog(player->GetSession()->GetPlayer(), "Player reached 1200h Playtime Reward");
		...
		if (playtime == 2400) {
            ...
            sPlayerLog->addCompletePlayerLog(player->GetSession()->GetPlayer(), "Player reached 2400h Playtime Reward");
			sendPlayerMailwithItem(itemid, itemamount, "Playtime Reward", message, player->GetSession()->GetPlayer());
			return;
		}
...
int CharacterSystemHandler::checkPlayerAccountSecurity(int accountid)
{
...
void CharacterSystemHandler::requestNewFirstCharacter(Player * player, const char * code)
{
	...
	if (!hasPlayerAlreadyAFirstChar(player->GetSession()->GetAccountId(), "FirstCharacter")) {
        sPlayerLog->addCompletePlayerLog(player->GetSession()->GetPlayer(), "Firstcharacter Request declined!");
	...
	hasPlayerMoreThanTwoFirstCharacters = countIfPlayerHasLessTotalOf2FirstCharacters(player->GetSession()->GetAccountId());
	if (hasPlayerMoreThanTwoFirstCharacters) {
		return;
	}

	std::string accountname = getAccountName(player->GetSession()->GetAccountId());
	sPlayerLog->addCompletePlayerLog(player->GetSession()->GetPlayer(), "Request new FirstChar and it is accepted!");
	std::string generatedCharacterName = generateNewCharacterName();
	std::string prefix = "first_";
	std::string newCharacterName = prefix + generatedCharacterName;
	deleteFirstCharacterPlayerLog(player->GetSession()->GetAccountId());
	updateCharacterToZeroAccount(newCharacterName, player->GetGUID());
...
bool CharacterSystemHandler::checkifPlayerisQualifiedforFirstCharacter(Player * player)
{
	bool hasPlayerAlreadyFirstChar = true;
...
void CharacterSystemHandler::sellPlayerVIPCurrency(Player * player, const char * code)
{
	...
	if (player->HasItemCount(changeitem, bonescost)){
		...
		sPlayerLog->addCompleteCurrencyLog(player->GetSession()->GetPlayer(), vipCoin, amount, bonescost, "VIP_CURRENCY_CREDIT at VIP_VENDOR");
        sPlayerLog->addCompletePlayerLog(player->GetSession()->GetPlayer(), "VIP_CURRENCY_CREDIT at VIP_VENDOR");
...
void CharacterSystemHandler::completeLearnProffesion(Player * player, SkillType skill,std::string Logmessage)
{
	int vipCoin = sConfigMgr->GetIntDefault("Vip.Vendor.CurrencyID", 38186);
	...
	if (player->HasItemCount(vipCoin, 4, false)) {
		...
		if (PlayerAlreadyHasTwoProfessions(player) && !IsSecondarySkill(skill)) {
			...
			ChatHandler(player->GetSession()).PSendSysMessage("##########################################################",
				player->GetName());
            sPlayerLog->addCompletePlayerLog(player->GetSession()->GetPlayer(), "Professionproblem caused with 2 Professions!");
			return;
		}
		...
		else
		{
			if (!LearnAllRecipesInProfession(player, skill)) {
				...
				ChatHandler(player->GetSession()).PSendSysMessage("##########################################################",
					player->GetName());
                sPlayerLog->addCompletePlayerLog(player->GetSession()->GetPlayer(), "Professionsystem Internal Server Error!");
				return;
			}
            ...
            sPlayerLog->addCompletePlayerLog(player->GetSession()->GetPlayer(), Logmessage);
			return;
		}
	}
	else {
		...
		ChatHandler(player->GetSession()).PSendSysMessage("##########################################################",
			player->GetName());
        sPlayerLog->addCompletePlayerLog(player->GetSession()->GetPlayer(), "Not enough coins for proffessions!");
		return;
	}
}
...
bool CharacterSystemHandler::PlayerAlreadyHasTwoProfessions(const Player *pPlayer)
{
	uint32 skillCount = 0;
...
bool  CharacterSystemHandler::LearnAllRecipesInProfession(Player *pPlayer, SkillType skill)
{
...
bool CharacterSystemHandler::IsSecondarySkill(SkillType skill)
{
...
uint32 CharacterSystemHandler::PlayerMaxLevel()
{
	return sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL);
}

int CharacterSystemHandler::countFBEventAccounts(Player * player)
{
...
void CharacterSystemHandler::insFBEvent(Player * player)
{
	std::string accountname = getAccountName(player->GetSession()->GetAccountId());
	SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
bool CharacterSystemHandler::isEventActive(int eventid)
{
...
void CharacterSystemHandler::eventNPCAI(int eventid,  Creature * creature)
{
...
void CharacterSystemHandler::givePlayerLevelWithCurrency(Player * player, uint16 cost, uint32 levelup,std::string logmessage)
{
...
void CharacterSystemHandler::executeGuildCharacter(Player * player, std::string playerlog, int membercount)
{
...
int CharacterSystemHandler::getUnixTimestamp()
{
...
void CharacterSystemHandler::sendPlayerMailwithItem(int itemid, int quantity,std::string title, std::string message, Player * player)
{
...
void CharacterSystemHandler::sendPlayerMailwithoutanyhing(Player * player, std::string title, std::string message)
{
	SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
void CharacterSystemHandler::sendPlayerMailwithGold(std::string title, std::string message, Player * player, int money)
{
	SQLTransaction trans = CharacterDatabase.BeginTransaction();
...
int CharacterSystemHandler::getAccountID(std::string charactername) {
...
std::string CharacterSystemHandler::getAccountName(int accountid) {
...
PreparedQueryResult CharacterSystemHandler::getCharacterByName(std::string charactername)
{
	PreparedStatement* charselbyname = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_BYNAME);
	charselbyname->setString(0, charactername);
	PreparedQueryResult result = CharacterDatabase.Query(charselbyname);
...
int CharacterSystemHandler::getGuildLeader(int guildid)
{
